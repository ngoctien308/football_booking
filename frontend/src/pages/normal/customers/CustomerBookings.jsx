...
const CustomerBookings = () => {
  ...
  const handlePay = async (bookingId) => {
    ...
    try {
      ...
      toast.success("Thanh toán giả thành công.");
    } catch (err) {
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-slate-800">Lịch đặt của tôi</h1>
            ...
            <div key={booking.id} className="bg-white border border-slate-200 rounded-xl p-4">
              ...
              <div className="mt-3 flex items-center justify-between text-xs sm:text-sm text-slate-600">
                ...
                {booking.status === "approved" && booking.payment_status !== "paid" && (
                  ...
                  >
                    {payingId === booking.id ? "Đang thanh toán..." : "Thanh toán (giả)"}
                  </button>
                )}
              </div>
            </div>
