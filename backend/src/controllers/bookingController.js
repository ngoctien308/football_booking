        ...
        // attach services to bookings
                ...
                if (bookingIds.length > 0) {
                    const { data: services, error: servicesError } = await db
                        ...
                        .select("id, booking_id, name, unit_price, quantity, total_price")
                        .in("booking_id", bookingIds);
                    if (servicesError) throw servicesError;
                    servicesMap = new Map();
                    for (const s of services || []) {
                        const arr = servicesMap.get(s.booking_id) || [];
                        arr.push(s);
                        servicesMap.set(s.booking_id, arr);
                    }
                }
                ...
                const rows = bookings.map((booking) => ({
                    ...booking,
                    field: fieldMap.get(booking.field_id) || null,
                    services: servicesMap.get(booking.id) || [],
                }));
...
export const createStripeCheckoutSession = async (req, res) => {
    try {
        ...
        const FRONTEND_BASE_URL = (process.env.FRONTEND_BASE_URL || "http://localhost:5173").replace(/\/$/, "");
                ...
                // fetch any services attached to this booking
                const { data: services, error: servicesError } = await db
                    .from("booking_services")
                    .select("id, booking_id, name, unit_price, quantity, total_price")
                ...
                const servicesTotal = (services || []).reduce((acc, s) => acc + Number(s.total_price || 0), 0);
                const fieldAmount = Math.max(0, Math.round(Number(booking.total_price || 0) - servicesTotal));
                ...
                if (fieldAmount > 0) {
                    line_items.push({
                        price_data: {
                            ...
                            product_data: {
                                name: `Đặt sân #${booking.id} - ${field?.field_name || "Sân"}`,
                                description: `Ngày ${booking.booking_date} | ${String(booking.start_time).slice(0, 5)} - ${String(
                                    booking.end_time
                                ).slice(0, 5)}`,
                            },
                ...
                for (const s of services || []) {
                    const unit = Math.max(0, Math.round(Number(s.unit_price || 0)));
                    const qty = Number(s.quantity || 1);
                    if (qty <= 0) continue;
                    // represent service as separate line item (unit price and quantity)
                    line_items.push({
                        price_data: {
                            currency: "vnd",
                            unit_amount: unit,
